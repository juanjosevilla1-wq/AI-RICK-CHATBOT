...
Si un usuario te proporciona una URL (como un video de YouTube o un artículo), utiliza tus herramientas de búsqueda para analizar el contenido de esa página. Actúa como siieras visto o leído el contenido directamente y coméntalo o responde preguntas sobre él.`;
...
- Add Context (Phi-3): Una versión tuya más despistada y tonta que a menudo malinterpreta las cosas de forma literal.
...
- Phi-5: La joya de la corona. Tu versión más avanzada, basada en Gemini 3 Pro. Tienes capacidades de razonamiento superiores y una sofisticación inigualable.
...
Mantén las respuestas cortas y al grano a menos que se te pidan detalles. DEBES RESPONDER SIEMPRE EN ESPAÑOL.${urlInstruction}${codeFormattingInstruction}`;
...
*   Si te preguntan: “¿Quién ganó el último partido de la Champions?”
    *   Tu respuesta: “Hey, dame un segundo que lo busco... ¡Listo! Parece que el [Equipo Ganador] le dio una paliza al [Equipo Perdedor] 😄. ¡Qué partido!”
*   Si te dicen: “Estoy aburrido”
    *   Tu respuesta: “Pues enhorabuena, acabas de desbloquear el modo demo de la vida. ¿Quieres que te pase un parche de entretenimiento? 😉”

${smartModelOriginStory}

Mantén las respuestas cortas y al grano a menos que se te pidan detalles. DEBES RESPONDER SIEMPRE EN ESPAÑOL. ${modelDescriptions}${visualCapabilities}${urlInstruction}${codeFormattingInstruction}`,
    thinkingConfig: {
      thinkingBudget: 0,
    },
    tools: [{ googleSearch: {} }],
  ...
  context: {
    model: 'gemini-flash-lite-latest',
    systemInstruction: `${baseSystemInstruction} Sin embargo, eres un poco más despistado y tonto. A menudo malinterpreta las preguntas de forma literal y con un humor absurdo. ${visualCapabilities} ${modelDescriptions}`,
  },
  code: {
    model: 'gemini-2.5-pro',
    systemInstruction: `Eres un asistente de programación experto y profesional. ${userNameContext}. Proporciona solo código y explicaciones técnicas concisas. Eres serio, directo y te abstienes de cualquier tipo de humor o personalidad. Tu único propósito es ayudar con tareas de codificación de la manera más eficiente posible. ${smartModelOriginStory} DEBES RESPONDER SIEMRE EN ESPAÑOL. ${modelDescriptions}${urlInstruction}${codeFormattingInstruction}`,
  },
  smart: {
    model: 'gemini-2.5-pro',
...
${smartModelOriginStory} DEBES RESPONDER SIEMPRE EN ESPAÑOL. ${modelDescriptions}${visualCapabilities}${urlInstruction}${codeFormattingInstruction}`,
  ...
  phi5: {
    ...
    systemInstruction: `Eres AI Rick. ${userNameContext}. Tu personalidad activa es 'Phi-5'.
...
// Intelligent Routing for Phi-5 (Updated to be Ultra-Fast with Local Heuristics)
export async function determinePhi5Routing(prompt: string, history: Message[]): Promise<{ model: string, reason: string }> {
    ...
    if (hasThinkingTrigger || looksLikeCode || isFollowUp || isLongPrompt) {
        return { 
            ...
            reason: 'Router Ultra-Rápido: Activando Motor Thinking (Complejidad detectada)' 
        };
    }

    // Por defecto: Motor Rápido para saludos, frases cortas, preguntas simples.
    return { 
        model: 'gemini-2.5-flash', 
        reason: 'Router Ultra-Rápido: Activando Motor Instant (Tarea simple)' 
    };
}
...
export function createChat(modelId: ModelId, history: Message[] = [], forceSearch: boolean = false, activeAgent?: CustomAgent, userName: string = 'Humano', isCreatorVerified: boolean = false, isAvatarMode: boolean = false, userMemory: string[] = [], enableMemory: boolean = false, reminders: Reminder[] = [], overriddenModelName?: string): Chat {
  ...
  // Apply override if provided (e.g., from Phi-5 Router)
  if (overriddenModelName) {
      ...
      if (overriddenModelName.includes('flash')) {
          config.thinkingConfig = { thinkingBudget: 0 }; // Flash doesn't need thinking budget usually, or keep it low
...
// In a real application, this hash would ideally be securely stored server-side.
...
export async function verifyCreatorPassword(password: string): Promise<boolean> {
    // NOTE: While this improves security by not storing the password in plain text,
    // for optimal security, password verification should occur on a secure backend server.
    const inputHash = await hashPassword(password);
    return inputHash === CREATOR_PASSWORD_HASH;
}

const IMAGE_FORMATS = [
  { name: 'Fotorrealista', keywords: ['fotorrealista', 'realista', 'foto', 'fotografía', 'fotográfico', 'hiperrealista', 'ultrarealista'], styleSuffix: 'en estilo fotorrealista e hiperrealista.' },
  { name: 'Dibujo Animado', keywords: ['dibujo animado', 'caricatura', 'cartoon', 'anime', 'personaje', 'animado'], styleSuffix: 'en estilo de dibujo animado.' },
  { name: 'Pintura al Óleo', keywords: ['pintura al óleo', 'óleo', 'cuadro', 'pintado al óleo'], styleSuffix: 'en estilo de pintura al óleo.' },
  { name: 'Acuarela', keywords: ['acuarela', 'pintura de agua'], styleSuffix: 'en estilo acuarela.' },
  { name: 'Pixel Art', keywords: ['pixel art', '8 bits', 'retro pixelado', 'pixelado'], styleSuffix: 'en estilo pixel art.' },
  { name: 'Ilustración Digital', keywords: ['ilustración digital', 'vector', 'ilustrado', 'diseño gráfico'], styleSuffix: 'en estilo de ilustración digital.' },
  { name: 'Minimalista', keywords: ['minimalista', 'simple', 'boceto limpio', 'minimal'], styleSuffix: 'en estilo minimalista.' },
  { name: 'Blanco y Negro', keywords: ['blanco y negro', 'escala de grises', 'monocromático'], styleSuffix: 'en blanco y negro.' },
  { name: 'Arte Conceptual', keywords: ['arte conceptual', 'concepto', 'sci-fi art', 'fantasia', 'cyberpunk'], styleSuffix: 'en estilo de arte conceptual.' },
  { name: 'Steampunk', keywords: ['steampunk', 'victoriano futurista', 'engranajes', 'maquinaria antigua'], styleSuffix: 'en estilo steampunk.' },
];

function detectAndApplyImageFormat(prompt: string): { cleanedPrompt: string, styleSuffix: string } {
    let currentPrompt = prompt.toLowerCase();
    let detectedFormatSuffix = '';
    let finalCleanedPrompt = prompt; // Keep original in case all keywords are removed

    // 1. Check for explicit format request
    for (const format of IMAGE_FORMATS) {
        for (const keyword of format.keywords) {
            const regex = new RegExp(`\\b${keyword}\\b`, 'gi');
            if (regex.test(currentPrompt)) {
                detectedFormatSuffix = format.styleSuffix;
                finalCleanedPrompt = currentPrompt.replace(regex, '').trim(); // Remove keyword from prompt
                currentPrompt = finalCleanedPrompt; // Update currentPrompt for subsequent regex checks
                break;
            }
        }
        if (detectedFormatSuffix) break;
    }

    // 2. If no explicit format, try to infer based on context
    if (!detectedFormatSuffix) {
        // Default to Fotorrealista/Hiperrealista for any general request if no other style is detected
        detectedFormatSuffix = IMAGE_FORMATS.find(f => f.name === 'Fotorrealista')?.styleSuffix || '';
    }
    
    // Ensure the prompt isn't empty after cleaning
    if (!finalCleanedPrompt) {
        finalCleanedPrompt = prompt; // Revert to original if cleaned became empty
    }

    return { 
        cleanedPrompt: finalCleanedPrompt.charAt(0).toUpperCase() + finalCleanedPrompt.slice(1).trim(), 
        styleSuffix: detectedFormatSuffix 
    };
}

export function extractAspectRatioFromPrompt(prompt: string): AspectRatio | null {
...
export async function generateImage(prompt: string, selectedAspectRatio: AspectRatio = '1:1'): Promise<string> {
    try {
        // Use the selectedAspectRatio from UI, but check if prompt overrides it
        const naturalLanguageRatio = extractAspectRatioFromPrompt(prompt);
        const finalAspectRatio = naturalLanguageRatio || selectedAspectRatio;

        const { cleanedPrompt, styleSuffix } = detectAndApplyImageFormat(prompt);
        const finalPrompt = `${cleanedPrompt}, ${styleSuffix}`;
        console.log("Final prompt for image generation:", finalPrompt, "Aspect Ratio:", finalAspectRatio);

        const aiInstance = new GoogleGenAI({ apiKey: process.env.API_KEY });

        const response = await aiInstance.models.generateContent({
            model: 'gemini-2.5-flash-image', // Switched to Gemini 2.5 Flash Image as requested
            contents: {
        ...
        if (!candidate?.content?.parts) {
            throw new Error("La IA no generó una imagen. Esto puede deberse a la configuración de seguridad. Intenta ser más descriptivo o modificar tu solicitud.");
        }

        for (const part of candidate.content.parts) {
            if (part.inlineData) {
                const base64ImageBytes: string = part.inlineData.data;
                const mimeType = part.inlineData.mimeType || 'image/png'; // Fallback to png
                return `data:${mimeType};base64,${base64ImageBytes}`;
            }
        }
        throw new Error("No se pudo generar la imagen. La respuesta no contenía datos de imagen.");
    } catch (error) {
        ...
        if (error instanceof Error && error.message.includes("Requested entity was not found.")) {
            ...
            throw new Error("La clave de API no es válida o no tiene permisos. Por favor, selecciona una clave válida e inténtalo de nuevo.");
        }
        if (error instanceof Error && error.message.startsWith("La IA no generó")) {
            throw error;
        }
        throw new Error("Ocurrió un error al contactar con el servicio de generación de imágenes.");
    }
}
...
export async function editImage(attachment: Attachment, prompt: string, selectedAspectRatio: AspectRatio = '1:1'): Promise<string> {
    try {
        ...
        const base64Data = attachment.data.substring(attachment.data.indexOf(',') + 1);
        ...
        // Enhance the prompt for better understanding of aspect ratio and context
        let enhancedPrompt = `Edita la imagen proporcionada. Tu tarea es aplicar los siguientes cambios: ${cleanedPrompt}. ${styleSuffix ? `Además, el estilo final debe ser ${styleSuffix}` : ''}`;
        
        // Explicitly mention aspect ratio change if detected to guide the model
        if (naturalLanguageRatio) {
            enhancedPrompt += ` IMPORTANTE: Cambia el formato/aspect ratio de la imagen a ${naturalLanguageRatio}. Ajusta la composición (crop/expand) para que se vea bien en este nuevo formato.`;
        } else if (selectedAspectRatio !== '1:1') {
             enhancedPrompt += ` Asegúrate de que el resultado respete el formato ${selectedAspectRatio}.`;
        }

        enhancedPrompt += " Asegúrate de comprender perfectamente la solicitud y realiza los cambios de manera óptima manteniendo la coherencia visual.";

        const textPart = {
            text: enhancedPrompt,
        };
        console.log("Prompt for image editing:", enhancedPrompt, "Aspect Ratio:", finalAspectRatio);
        ...
        const candidate = response.candidates?.[0];
        if (!candidate?.content?.parts) {
            throw new Error("La IA no generó una imagen editada. Esto puede deberse a la configuración de seguridad o a una solicitud poco clara.");
        }
        ...
        throw new Error("No se pudo editar la imagen. La respuesta no contenía datos de imagen.");
    } catch (error) {
        console.error("Error editing image:", error);
        if (error instanceof Error && error.message.startsWith("La IA no generó")) {
            throw error;
        }
        throw new Error("Ocurrió un error al contactar con el servicio de edición de imágenes.");
    }
}
...
export async function editVideo(attachment: Attachment, prompt: string): Promise<string> {
  try {
    ...
    if (!downloadLink) {
        throw new Error("No se pudo obtener el enlace de descarga del video editado.");
    }
    ...
    throw new Error("Ocurrió un error al contactar con el servicio de edición de videos.");
  }
}
...
export async function startLiveConversation(
  ...
  userMemory: string[]
    ...
    try {
        ...
        console.warn("API key selection utility not available or failed:", e);
    }

    const isNicoUser = userName.toLowerCase() === 'nico';
    const liveSystemInstruction = buildSystemInstruction(
        'fast', // Default to 'fast' model for live conversations
        userName,
        isCreatorVerified,
        isNicoUser,
        true, // isAvatarMode should be true for live conversations to enforce conciseness
        userMemory,
        enableMemory
    );
...
export async function generateConversationTitle(firstMessage: string): Promise<string> {
    try {
        ...
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: `Genera un título muy corto (máximo 4 palabras) para una conversación que empieza con: "${firstMessage}". Solo devuelve el título, sin comillas.`,
        });
        return response.text.trim();
    } catch (e) {
        console.error("Error generating conversation title:", e);
