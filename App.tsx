...
const MODELS: Model[] = [
  ...
  { id: 'phi5', name: 'Phi-5.1🚀', badge: 'NUEVO' },
...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (userInput: string, userName: string) => {
    ...
    else if (currentModel === 'phi5') setProcessingMessage("Phi-5.1: Analizando complejidad...");
    ...
    try {
        ...
        if (isImageEditing && lastImageInHistory) {
            ...
            if (effectiveModelId === 'phi5' && !activeAgent) {
                ...
                console.log("Phi-5.1 Routing Decision:", routingResult);
                
                if (specificModelName.includes('flash')) {
                    setProcessingMessage("Phi-5.1: Modo Rápido (Flash) activado ⚡");
                } else {
                    setProcessingMessage("Phi-5.1: Modo Razonamiento (Pro) activado 🧠");
                }
            ...
            if (aggregatedToolCalls.length > 0) {
                ...
                setConversations(convos => [currentConvoId].map(c => {
                    if (c !== currentConvoId) return c;
                    const updatedMessages = [...conversations.find(conv => conv.id === currentConvoId)!.messages];
                    updatedMessages[updatedMessages.length - 1] = finalModelMessage;
                    return { ...conversations.find(conv => conv.id === currentConvoId)!, messages: updatedMessages };
                }));
